        ...
        private void SendToTCP()
        {
            if (NetworkInterface.GetIsNetworkAvailable())
            {
                try
                {
                    ...
                    PingReply pingReply = serverPing.Send(_ipAddress, _portNumber);
                    if (pingReply.Address != null)
                    {
                        MessageBox.Show($"You have stablish connection with {_planetName} planet!");
                        _isConnected = true;
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Error when pinging {_planetName} planet: " + ex.Message);
                }
            }
        }

        private void SendToTCP(string message)
        {
            if (!string.IsNullOrWhiteSpace(txb_message.Text))
            {
                _tcpClient = new TcpClient(_ipAddress.ToString(), _portNumber);

                Byte[] messageIntoBytes = Encoding.ASCII.GetBytes(message);
                _networkStream = _tcpClient.GetStream();

                _networkStream.Write(messageIntoBytes, 0, messageIntoBytes.Length);
            }
            else
            {
                MessageBox.Show("Can't send empty message to the server");
